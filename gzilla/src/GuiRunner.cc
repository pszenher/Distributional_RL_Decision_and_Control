...
public:
  std::unique_ptr<SystemLoader> systemLoader;

  /// \brief Mutex to protect systemLoader
public:
  std::mutex systemLoadMutex;

  /// \brief Events containing visual plugins to load
public:
  std::vector<std::pair<gz::sim::Entity, sdf::Plugin>> visualPlugins;
...
public:
  ...
  /// \brief Systems implementing PreUpdate
public:
  std::vector<ISystemPreUpdate *> systemsPreupdate;

  /// \brief Systems implementing Update
public:
  std::vector<ISystemUpdate *> systemsUpdate;

  /// \brief Systems implementing PostUpdate
public:
  std::vector<ISystemPostUpdate *> systemsPostupdate;
...
: dataPtr(utils::MakeUniqueImpl<Implementation>())
{
  ...
  this->dataPtr->stateTopic =
    transport::TopicUtils::AsValidTopic("/world/" + _worldName + "/state");
  if (this->dataPtr->stateTopic.empty()) {
    gzerr << "Failed to generate valid topic for world [" << _worldName << "]" << std::endl;
    return;
  }

  common::addFindFileURICallback(
    [](common::URI _uri) { return fuel_tools::fetchResource(_uri.Str()); });

  gzdbg << "Requesting initial state from [" << this->dataPtr->stateTopic << "]..." << std::endl;
  ...
  timer->start(GZ_GUI_PLUGIN_UPDATE_PERIOD);
...
bool GuiRunner::eventFilter(QObject * _obj, QEvent * _event)
{
  if (_event->type() == gz::gui::events::WorldControl::kType) {
    auto worldControlEvent = reinterpret_cast<gz::gui::events::WorldControl *>(_event);
    if (worldControlEvent) {
      msgs::WorldControlState req;
      req.mutable_world_control()->CopyFrom(worldControlEvent->WorldControlInfo());
      ...
      std::function<void(const gz::msgs::Boolean &, const bool)> cb =
        [](const gz::msgs::Boolean & /*_rep*/, const bool _result) {
          if (!_result) gzerr << "Error sharing WorldControl info with the server.\n";
        };
      this->dataPtr->node.Request(this->dataPtr->controlService, req, cb);
    }
  } else if (_event->type() == gz::sim::gui::events::VisualPlugins::kType) {
    auto visualPluginEvent = reinterpret_cast<gui::events::VisualPlugins *>(_event);
    if (visualPluginEvent) {
      ...
      for (const sdf::Plugin & plugin : visualPluginEvent->Plugins()) {
        this->dataPtr->visualPlugins.emplace_back(entity, plugin);
      }
    }
  }
...
void GuiRunner::RequestState()
{
  ...
  std::string reqSrv = this->dataPtr->node.Options().NameSpace() + "/" + id + "/state_async";
  auto reqSrvValid = transport::TopicUtils::AsValidTopic(reqSrv);
  if (reqSrvValid.empty()) {
    gzerr << "Failed to generate valid service [" << reqSrv << "]" << std::endl;
    return;
  }
  reqSrv = reqSrvValid;

  auto advertised = this->dataPtr->node.AdvertisedServices();
  if (std::find(advertised.begin(), advertised.end(), reqSrv) == advertised.end()) {
    if (!this->dataPtr->node.Advertise(reqSrv, &GuiRunner::OnStateAsyncService, this)) {
  ...
  this->dataPtr->node.Subscribe(this->dataPtr->stateTopic, &GuiRunner::OnState, this);
...
void GuiRunner::OnStateAsyncService(const msgs::SerializedStepMap & _res)
{
  ...
  QMetaObject::invokeMethod(
    this, "OnStateQt", Qt::QueuedConnection, Q_ARG(msgs::SerializedStepMap, _res));
...
void GuiRunner::OnState(const msgs::SerializedStepMap & _msg)
{
  ...
  if (!this->dataPtr->receivedInitialState) return;
  ...
  // variables.
  QMetaObject::invokeMethod(
    this, "OnStateQt", Qt::QueuedConnection, Q_ARG(msgs::SerializedStepMap, _msg));
}
  ...
  for (auto & visualPlugin : this->dataPtr->visualPlugins) {
    ...
    if (plugin.Filename() != "__default__" && plugin.Name() != "__default__") {
      ...
      if (system) {
        ...
        this->dataPtr->systemsPreupdate.push_back(sys->QueryInterface<ISystemPreUpdate>());
        this->dataPtr->systemsUpdate.push_back(sys->QueryInterface<ISystemUpdate>());
        this->dataPtr->systemsPostupdate.push_back(sys->QueryInterface<ISystemPostUpdate>());

        auto sysConfigure = sys->QueryInterface<ISystemConfigure>();
        if (sysConfigure) {
          sysConfigure->Configure(
            entity, plugin.ToElement(), this->dataPtr->ecm, this->dataPtr->eventMgr);
        }
        gzdbg << "Loaded system [" << plugin.Name() << "] for entity [" << entity << "] in GUI"
              << std::endl;
      }
    }
  }
...
void GuiRunner::UpdateSystems()
{
...

  {
    ...
    for (auto & system : this->dataPtr->systemsPreupdate) {
      if (system) system->PreUpdate(this->dataPtr->updateInfo, this->dataPtr->ecm);
    }
  }

  {
    ...
    for (auto & system : this->dataPtr->systemsUpdate) {
      if (system) system->Update(this->dataPtr->updateInfo, this->dataPtr->ecm);
    }
  }

  {
    ...
    for (auto & system : this->dataPtr->systemsPostupdate) {
      if (system) system->PostUpdate(this->dataPtr->updateInfo, this->dataPtr->ecm);
    }
  }
}

/////////////////////////////////////////////////
EventManager & GuiRunner::GuiEventManager() const { return this->dataPtr->eventMgr; }
