...
std::unique_ptr<gz::gui::Application> GzGui::CreateGui(std::string worldName)
{
  ...
  auto app = std::make_unique<gz::gui::Application>(
    argc, argv.data(), gz::gui::WindowType::kDialog,
  ...
  // if (!ament_index_cpp::get_resource( "vendor_packages", packageName,
  // defaultConfig, &configPrefix )) {
  //   RCLCPP_ERROR(this->get_logger(),
  // 		   "Failed resolve ROS package '%s', cannot load default
  // 'gui.config' file", 		   packageName.c_str());
  //   return nullptr;
  // }

  // // FIXME: last component should generalize over version (currently
  // hardcodes `gz-sim8`) defaultConfig = configPrefix + "/" + defaultConfig +
  // "/share/gz/gz-sim8/gui/gui.config";

  std::string defaultConfig =
    ament_index_cpp::get_package_share_directory("gzilla") + "/config/gui_minimal.config";

  RCLCPP_INFO(this->get_logger(), "Default config resolved");

  // gz::common::env(GZ_HOMEDIR, defaultConfig);
  // defaultConfig = gz::common::joinPaths(defaultConfig, ".gz", "sim",
  // GZ_SIM_MAJOR_VERSION_STR, "gui.config");
  ...
  if (!app->CreateMainWindow()) {
    RCLCPP_ERROR(this->get_logger(), "Failed to initialize main gazebo window");
    return nullptr;
  }
  ...
  if (!app->LoadConfig(defaultConfig)) {
    RCLCPP_ERROR(this->get_logger(), "Failed load config file '%s'", defaultConfig.c_str());
    return nullptr;
  }
  RCLCPP_INFO(this->get_logger(), "loaded config");

  auto runner = std::make_shared<gz::sim::GuiRunner>(worldName);
...
void GzGui::OnStart()
{
  ...
  if (world_name.empty()) {
    RCLCPP_ERROR(this->get_logger(), "Must specify 'world_name'");
    rclcpp::shutdown();
    return;
  }
  ...
  if (verbosity < 1 || GZ_MAX_LOG_LEVEL < verbosity) {
    ...
    RCLCPP_WARN(
      this->get_logger(),
      "Out-of-range verbosity level provided (verbosity=%ld);  "
      "clamping to nearest:  %ld",
  ...
  if (!app) {
    RCLCPP_ERROR(this->get_logger(), "Failed to create Gazebo GUI instance");
    rclcpp::shutdown();
    return;
  }
